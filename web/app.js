...
async function handleReceipt(event) {
  ...
  reader.onload = async () => {
    try {
      ...
      notify("凭证已保存");
    } catch (error) { $("#upload-status").textContent = `上传失败：${error.message}`; }
  };
